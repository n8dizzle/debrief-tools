...
/**
 * Run the AR sync process
 * This is extracted to be callable from both the cron endpoint and the manual sync endpoint
 */
export async function runARSync(): Promise<SyncResult> {
  ...
  try {
    ...
    if (arRows.length === 0) {
      ...
      return {
        ...
        stats: {
          invoices: { processed: 0, created: 0, updated: 0, paid: 0 },
          customers: { processed: 0, created: 0, updated: 0 },
          payments: { processed: 0, created: 0 },
        },
        errors: [],
      };
    }
