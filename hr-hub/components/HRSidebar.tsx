...
const mainLinks = [
  ...
  { href: '/onboardings/new', label: 'New Onboarding', icon: 'plus' },
...
export default function HRSidebar({ isOpen = true, onClose }: HRSidebarProps) {
  const pathname = usePathname();
  const { canManageTemplates, canCreateOnboardings } = useHRPermissions();
  ...
  const visibleLinks = mainLinks.filter((link) => {
    if (link.href === '/templates' && !canManageTemplates) return false;
    if (link.href === '/onboardings/new' && !canCreateOnboardings) return false;
    return true;
  });
