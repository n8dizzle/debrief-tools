...
interface SendSMSResult { success: boolean; messageId?: string; error?: string }

export async function sendSMS(to: string, message: string): Promise<SendSMSResult> {
  if (!QUO_API_KEY || !QUO_FROM_NUMBER) return { success: false, error: 'Quo not configured' };
  const formattedTo = formatPhoneE164(to);
  if (!formattedTo) return { success: false, error: `Invalid phone number: ${to}` };
  try {
    const res = await fetch('https://api.quo.com/v1/messages', {
      method: 'POST',
      headers: { Authorization: QUO_API_KEY, 'Content-Type': 'application/json' },
      body: JSON.stringify({ content: message, from: QUO_FROM_NUMBER, to: [formattedTo], ...(QUO_USER_ID ? { userId: QUO_USER_ID } : {}) }),
    });
