 ...
 *
 * Validates a user session for the Python app SSO.
 ...
 */
export async function POST(request: NextRequest): Promise<NextResponse<SessionValidationResponse>> {
  ...
  try {
    ...
    if (!email) {
      return NextResponse.json(
        { valid: false, error: 'Email required for validation' },
        { status: 400 }
      );
    }
