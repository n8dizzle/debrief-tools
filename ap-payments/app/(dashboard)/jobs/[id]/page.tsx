            ...
            <div className="space-y-3">
              ...
              {(approvalChain.approved_by || approvalChain.paid_by) && (
                <div className="pt-1 space-y-1">
                  ...
                  {job.payment_confirmation_code && (
                    <div className="flex items-center gap-2 text-xs" style={{ color: 'var(--text-secondary)' }}>
                      <svg className="w-3.5 h-3.5 flex-shrink-0" style={{ color: 'var(--text-muted)' }} fill="none" stroke="currentColor" viewBox="0 0 24 24">
                        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M9 12h6m-6 4h6m2 5H7a2 2 0 01-2-2V5a2 2 0 012-2h5.586a1 1 0 01.707.293l5.414 5.414a1 1 0 01.293.707V19a2 2 0 01-2 2z" />
                      </svg>
                      <span>{job.payment_batch_id ? 'Lump payment' : 'Payment'} · confirmation <strong style={{ color: 'var(--text-primary)' }}>{job.payment_confirmation_code}</strong></span>
                    </div>
                  )}
                </div>
              )}
