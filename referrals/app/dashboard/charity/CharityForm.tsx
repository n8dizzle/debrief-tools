...
export default function CharityForm({ initialCharityId, charities }: Props) {
  ...
  const isOther = charityId === "other";
  const dirty = charityId !== initialCharityId;
  const canSave = dirty && !!charityId && (!isOther || suggestedName.trim().length > 0);
  ...
  async function save() {
    ...
    try {
      const res = await fetch("/api/dashboard/charity", {
        ...
        body: JSON.stringify({
          selectedCharityId: isOther ? null : charityId,
          suggestedCharityName: isOther ? suggestedName.trim() : null,
        }),
      });
    ...
    <div className="card">
      ...
      <div className="grid gap-3">
        {charities.map((c) => (
          <label
            ...
            style={{
              border: `2px solid ${charityId === c.id ? "var(--ca-green)" : "var(--border-subtle)"}`,
              background: charityId === c.id ? "rgba(97,139,96,0.06)" : "var(--bg-card)",
            }}
          >
            <input
              type="radio"
      ...
      {error && (
        <p className="mt-4 p-3 rounded-lg text-sm"
          style={{ background: "rgba(135,76,59,0.1)", color: "var(--ca-red)", border: "1px solid var(--ca-red)" }}>
          {error}
        </p>
      )}
      ...
      <div className="flex items-center justify-between mt-4">
        <p className="text-sm opacity-70">
          {savedAt && !dirty ? `Saved at ${savedAt.toLocaleTimeString()}` : ""}
        </p>
        <button
          className="btn btn-primary"
          onClick={save}
          disabled={!canSave || busy}
          style={{ opacity: !canSave || busy ? 0.5 : 1, cursor: !canSave || busy ? "not-allowed" : "pointer" }}
        >
