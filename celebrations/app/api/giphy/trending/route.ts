...
export async function GET(req: NextRequest) {
  try {
    ...
    const url = `https://api.giphy.com/v1/gifs/trending?api_key=${apiKey}&limit=${limit}&offset=${offset}&rating=pg`;
    const res = await fetch(url);

    if (!res.ok) {
      const text = await res.text();
      console.error('GIPHY trending error:', res.status, text);
    ...
    console.error('GIPHY trending exception:', err);
