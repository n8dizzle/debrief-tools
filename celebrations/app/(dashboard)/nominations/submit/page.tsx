...
export default function DashboardNominatePage() {
  ...
  const nominatorName = session?.user?.name || '';
    ...
    return (
      <div className="max-w-lg mx-auto py-16 text-center">
        ...
        <p className="mb-6" style={{ color: 'var(--text-secondary)' }}>
          Thank you for recognizing <strong style={{ color: 'var(--christmas-cream)' }}>{nomineeName}</strong>.
        </p>
  ...
  return (
    <div className="max-w-lg">
      ...
      <form onSubmit={handleSubmit} className="space-y-5">
        ...
        {/* Submitting as */}
        <div className="p-3 rounded-lg text-sm" style={{ background: 'var(--bg-card)', border: '1px solid var(--border-subtle)' }}>
          <span style={{ color: 'var(--text-muted)' }}>Submitting as </span>
          <span style={{ color: 'var(--christmas-cream)' }}>{nominatorName}</span>
        </div>
