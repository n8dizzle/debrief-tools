...
export default function EditPeriodPage() {
  ...
  const [status, setStatus] = useState<NominationPeriodStatus>('draft');
  ...
  useEffect(() => {
    async function load() {
      try {
        ...
        const p: CelNominationPeriod = data.period;
        setPeriodType(p.period_type || 'quarterly');
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      router.push(`/nominations/periods/${periodId}`);
    } catch (err: any) {
    ...
    <div className="max-w-2xl">
      ...
      <form onSubmit={handleSubmit} className="space-y-6">
        ...
        <div className="flex gap-3 pt-4">
          ...
          <button
            type="button"
            onClick={() => router.push(`/nominations/periods/${periodId}`)}
