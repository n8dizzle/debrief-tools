...
const nominationLinks = [
  { href: '/nominations', label: 'Nominations', icon: 'trophy', requiresManager: true },
];
...
function NavIcon({ type }: { type: string }) {
  const icons: Record<string, JSX.Element> = {
    ...
    trophy: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
        <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={1.5} d="M16 4h2a2 2 0 012 2v1a4 4 0 01-4 4m-6-7H6a2 2 0 00-2 2v1a4 4 0 004 4m0 0h4m-4 0v3a2 2 0 002 2h0a2 2 0 002-2v-3m0 0a4 4 0 004-4V6a2 2 0 00-2-2h-2m-4 0V3m0 1h4" />
      </svg>
    ),
    list: (
      <svg className="w-5 h-5" fill="none" stroke="currentColor" viewBox="0 0 24 24">
...
export default function CelebrationsSidebar({ isOpen = true, onClose }: CelebrationsSidebarProps) {
  ...
  const isActive = (href: string) => {
    ...
    if (href === '/nominations') return pathname === '/nominations' || pathname.startsWith('/nominations/periods');
  ...
  const filteredNominationLinks = nominationLinks.filter(
    link => !link.requiresManager || isManager || isOwner
  );
      ...
      >
        ...
        <nav className="flex-1 overflow-y-auto p-4">
          ...
          <div className="mb-6">
            ...
            <div className="space-y-1">
              {filteredNominationLinks.map((link) => (
                <Link
                  key={link.href}
                  href={link.href}
                  onClick={handleLinkClick}
