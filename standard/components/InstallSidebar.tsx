...
export default function InstallSidebar({
  ...
  return (
    <>
      ...
      <aside
        className={`fixed left-0 top-0 h-screen flex flex-col z-50 transform transition-all duration-300 ease-in-out lg:translate-x-0 ${isOpen ? 'translate-x-0' : '-translate-x-full'} ${collapsed ? 'w-20' : 'w-64'}`}
        style={{ backgroundColor: 'var(--bg-secondary)', borderRight: '1px solid var(--border-subtle)' }}
      >
        <div className={`p-4 border-b flex items-center ${collapsed ? 'justify-center' : 'justify-between'}`} style={{ borderColor: 'var(--border-subtle)' }}>
          <Link href="/" className="flex items-center gap-3 min-w-0" onClick={onClose} title={collapsed ? 'Christmas Air · Standard' : undefined}>
            ...
            {!collapsed && (
              <div className="min-w-0">
                <div className="font-bold text-lg truncate" style={{ color: 'var(--text-primary)' }}>Christmas Air</div>
                <div className="text-xs truncate" style={{ color: 'var(--text-secondary)' }}>Standard</div>
              </div>
            )}
          </Link>
          {!collapsed && onClose && (
            <button onClick={onClose} className="lg:hidden p-2 rounded-lg hover:bg-white/10 transition-colors shrink-0" style={{ color: 'var(--text-secondary)' }} aria-label="Close menu">
        ...
        {/* Desktop collapse toggle — mirrors the orders app */}
        {onToggleCollapsed && (
          <button
            onClick={onToggleCollapsed}
            className={`hidden lg:flex items-center px-4 py-2 border-b hover:bg-black/5 transition-colors ${collapsed ? 'justify-center' : 'justify-end'}`}
          ...
          <div className="space-y-1">
            {links.map((link) => (
              ...
              >
                ...
                {!collapsed && <span className={`text-sm ${isActive(link.href) ? 'font-medium' : ''}`}>{link.label}</span>}
              </Link>
            ))}
          </div>
        </nav>
