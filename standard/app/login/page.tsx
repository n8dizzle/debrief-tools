    ...
    <div className="login-card">
      ...
      {error && (
        <p className="login-error">
          {error === 'NotRegistered'
            ? "Your account isn't set up yet. Ask a manager for access."
            : error === 'AccountInactive'
            ? 'Your account has been deactivated. Contact a manager.'
            : error === 'AccessDenied'
      ...
      <p className="login-foot">Only @christmasair.com accounts can access this app.</p>
    </div>
  );
}
...
export default function LoginPage() {
  return (
    <div className="login-screen">
      <Suspense fallback={<p style={{ color: 'var(--ink-3)' }}>Loading…</p>}>
        <LoginContent />
      </Suspense>
    </div>
  );
