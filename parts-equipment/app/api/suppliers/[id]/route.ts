...
// PATCH /api/suppliers/[id] — rename and/or toggle active.
...
// DELETE /api/suppliers/[id] — remove a supplier. Existing orders keep their
// stored supplier name string, so this is safe.
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const supabase = getServerSupabase();
  const { error } = await supabase.from('pe_suppliers').delete().eq('id', id);
  if (error) {
