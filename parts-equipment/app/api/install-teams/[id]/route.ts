...
// PATCH /api/install-teams/[id] — rename and/or toggle active.
...
// DELETE /api/install-teams/[id] — remove a team. Existing orders keep their
// stored team name string, so this is safe.
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const supabase = getServerSupabase();
  const { error } = await supabase.from('pe_install_teams').delete().eq('id', id);
  if (error) {
