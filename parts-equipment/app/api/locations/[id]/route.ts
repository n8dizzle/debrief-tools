...
// PATCH /api/locations/[id] — rename and/or toggle active.
...
// DELETE /api/locations/[id] — remove a location. Existing orders keep their
// stored location string, so this is safe.
export async function DELETE(_request: NextRequest, { params }: { params: Promise<{ id: string }> }) {
  ...
  const supabase = getServerSupabase();
  const { error } = await supabase.from('pe_locations').delete().eq('id', id);
  if (error) {
