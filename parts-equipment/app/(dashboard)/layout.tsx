...
export default async function DashboardLayout({ children }: { children: React.ReactNode }) {
  ...
  if (user.role !== 'owner' && !user.permissions?.parts_equipment?.can_view) {
    return (
      <div style={{ minHeight: '100vh', display: 'flex', alignItems: 'center', justifyContent: 'center', backgroundColor: 'var(--bg-primary)', color: 'var(--text-primary)' }}>
        <div style={{ textAlign: 'center', maxWidth: 400 }}>
          ...
          <a href="https://portal.christmasair.com" style={{ color: 'var(--christmas-green)', textDecoration: 'underline' }}>
            Go to Portal
          </a>
        </div>
      </div>
